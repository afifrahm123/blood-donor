...
function NewBloodRequest() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/patient/blood-request`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate('/patient-dashboard/requests'), 2000);
      } else {
